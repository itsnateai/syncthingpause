...
- **"Windows startup delay" box is sized for the value, not the max.** Narrowed from 80px to 58px — it fits the realistic 1–2 digit delay (default 20s) instead of looking oversized, while still clearing the 3600 max + spinner (measured ~7px headroom at 100%; `AutoScaleMode.Dpi` scales the box linearly, so the fit holds at higher scales), so the `[0, 3600]` range the load clamp depends on is unchanged.
...
v3.2.8 scrubbed personal identifiers from the release surface. This release continues that pass by removing two more classes of leak that v3.2.8 missed:

1. **AI-tooling fingerprints** in CHANGELOG entries and source comments (specific model names attributing the verifier methodology to a particular AI provider).
2. **Private wiki path references** in source comments (paths to a private development wiki that don't exist in the public repo and 404 for any reader).

### What's underneath

- **`CHANGELOG.md`** — v3.0.0, v3.0.1, v3.2.0, v3.2.5, v3.2.7 verifier-coverage entries rewritten to describe rounds and pair structure without naming the underlying AI models. Technical content (round counts, what each round caught, lessons learned) unchanged. The v3.2.0 entry's "Pattern lifted from the canonical …" line also dropped the private-wiki path token.
- **`SettingsForm.cs:66, :1266`** — comment scrubs.
- **`OsdToolTip.cs:21`** — comment scrub.
- **`AppConfig.cs:32`** — private-wiki filename removed from the user-facing surface enumeration.
- **`TrayIconPromoter.cs:47`** — private-path `Canonical snippet:` reference removed; the class itself is the canonical implementation in this repo.
- **`TrayApplicationContext.cs:247, :3213, :3523`** — private-path snippet references removed; named-tool enumeration in the GOMAXPROCS rationale generalized to "multiple developer tools".
- **GitHub release notes for v1.4.0 / v3.0.0 / v3.0.1 / v3.1.0 / v3.2.0 / v3.2.5 / v3.2.7** — body text on `github.com/itsnateai/syncthingpause/releases` rewritten to match the further-scrubbed CHANGELOG. (v3.2.8's release body is auto-generated by CI from the CHANGELOG v3.2.8 section and is already clean; v1.4.0's body dropped one bullet that named the private-wiki filename.)
- **`SyncthingPause.csproj`** — 3.2.8 → 3.2.9.

### Verifier coverage

Build clean (0 warnings, 0 errors). 92/92 tests pass. A repository-wide grep against tracked files for the scrubbed AI-tooling names and private-wiki path tokens now returns only the single `.gitignore` entry that names the private-wiki file to exclude it from commits (structurally required — cannot be removed without losing the privacy protection it provides). All other matches across `*.cs`, `*.md`, `*.csproj`, `*.yml`, `*.yaml` cleared.
...
This is the second time in this session a verifier convergence (two independent verifier rounds agreeing) turned out to be wrong. Round 1's "post-Add design-literal overwrites autoscaled Width" finding was load-bearing only IF AutoScale had already fired — which it hasn't, at ctor time, on a Form whose handle isn't created yet. Both verifier rounds applied the same flawed assumption. Lesson: agent convergence is signal that something is **interesting**, not signal that the finding is **correct**. Empirical test (or in this case: deploying to a 125 % screen and observing the broken layout) is the only ground truth.
...
- Pattern lifted from a sibling tray app's user-selectable-theme implementation, which documents both Variant A (Catppuccin Latte) and Variant B (v2.1.x classic, what we shipped). v3.2.0 is the second adopter of the v2.1.x classic variant after MWBToggle v2.5.18.
...
- 8-agent verifier sweep (two independent verifier pairs on each of: diff-clean, gap-audit, code-review, blast-radius) — closed 6 convergent findings: WaitForExit after Kill (race-window fix), KilledRenamePredecessor wired to OSD, INI Copy reparse-point + 256 KB size cap, portable-mode guard around rename-predecessor pause.dat migration, RunOnStartup .lnk recreation when legacy .lnk existed, this test count.
